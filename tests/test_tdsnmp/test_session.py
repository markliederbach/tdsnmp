...
Tests for `tdsnmp` module.
"""
import re
import platform
import pytest

from tdsnmp import exceptions
from tdsnmp.session.base import Session
...
def reset_values():
    # TODO: Turn this on once localhost SNMP is working
    #snmp_set_via_cli('sysLocation.0', 'my original location', 's')
    #snmp_set_via_cli('nsCacheTimeout.1.3.6.1.2.1.2.2', '0', 'i')
    yield

def test_session_000_invalid_version():
    with pytest.raises(exceptions.UnsupportedSNMPVersion):
        Session(version=4)

@pytest.mark.parametrize('version', [1, 2, 3])
def test_session_001_invalid_hostname(version):
    with pytest.raises(exceptions.TDSNMPConnectionError):
        sess = Session(hostname='invalid', version=version)
        sess.get('sysContact.0')

@pytest.mark.parametrize('version', [1, 2, 3])
def test_session_002_invalid_hostname_and_port(version):
    with pytest.raises(exceptions.ImproperlyConfigured):
        Session(hostname='localhost:162', remote_port=163, version=version)

@pytest.mark.parametrize('version', [1, 2, 3])
def test_session_003_hostname_and_port_split(version):
    sess = Session(hostname='localhost:162', version=version)
    assert sess.hostname == 'localhost'
    assert sess.remote_port == 162

@pytest.mark.parametrize('version', [1, 2, 3])
def test_session_004_invalid_port(version):
    with pytest.raises(exceptions.TDSNMPTimeoutError):
        sess = Session(remote_port=1234, version=version, timeout=0.2, retries=1)
        sess.get('sysContact.0')

@pytest.mark.parametrize('sess', [sess_v1()])
def test_session_005_set_multiple(sess):
...
def test_session_006_set_clear(sess):
...
@pytest.mark.parametrize('sess', [sess_v1(), sess_v2(), sess_v3()])
def test_session_008_get_use_numeric(sess):
...
@pytest.mark.parametrize('sess', [sess_v1(), sess_v2(), sess_v3()])
def test_session_009_get_use_sprint_value(sess):
...
@pytest.mark.parametrize('sess', [sess_v1(), sess_v2(), sess_v3()])
def test_session_010_get_use_enums(sess):
    sess.use_enums = True
...
@pytest.mark.parametrize('sess', [sess_v1(), sess_v2(), sess_v3()])
def test_session_011_get_next(sess):
    res = sess.get_next([
        ('sysUpTime', '0'),
        ('sysContact', '0'),
        ('sysLocation', '0')
...
@pytest.mark.parametrize('sess', [sess_v1(), sess_v2(), sess_v3()])
def test_session_013_set_multiple(sess):
...
@pytest.mark.parametrize('sess', [sess_v1(), sess_v2(), sess_v3()])
def test_session_014_get_bulk(sess):  # noqa
    if sess.version == 1:
        with pytest.raises(exceptions.TDSNMPException):
            sess.get_bulk(
...
@pytest.mark.parametrize('sess', [sess_v1(), sess_v2(), sess_v3()])
def test_session_016_get_invalid_instance_with_abort_enabled(sess):
    # Sadly, SNMP v1 doesn't distuingish between an invalid instance and an
    # invalid object ID, instead it excepts with noSuchName
    ...
    if sess.version == 1:
        ...
        with pytest.raises(exceptions.TDSNMPNoSuchInstanceError):
            sess.get('sysDescr.100')


@pytest.mark.parametrize('sess', [sess_v1(), sess_v2(), sess_v3()])
def test_session_017_get_invalid_object(sess):
    if sess.version == 1:
        with pytest.raises(exceptions.TDSNMPNoSuchNameError):
            sess.get('iso')
    else:
...
@pytest.mark.parametrize('sess', [sess_v1(), sess_v2(), sess_v3()])
def test_session_018_get_invalid_object_with_abort_enabled(sess):
    sess.abort_on_nonexistent = True
    if sess.version == 1:
        with pytest.raises(exceptions.TDSNMPNoSuchNameError):
            sess.get('iso')
    else:
        with pytest.raises(exceptions.TDSNMPNoSuchObjectError):
            sess.get('iso')

@pytest.mark.parametrize('sess', [sess_v1(), sess_v2(), sess_v3()])
def test_session_019_walk(sess):
...
@pytest.mark.parametrize('sess', [sess_v1(), sess_v2(), sess_v3()])
def test_session_020_bulkwalk(sess):
    if sess.version == 1:
        with pytest.raises(exceptions.TDSNMPException):
...
@pytest.mark.parametrize('sess', [sess_v1(), sess_v2(), sess_v3()])
def test_session_021_walk_all(sess):
